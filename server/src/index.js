...
import contactosRouter from './routes/contacto.js'; 
...
app.get("/", (req, res) => {
    res.sendFile(path.join(__dirname, '../client/HTML/index.html')); // Cambiado a index.html
});
...
// Ruta para obtener zapatillas con filtros y ordenación
app.get('/api/zapatillas', async (req, res) => {
    try {
		...
		if (talle) whereClause.talle = talle;
		if (marca) whereClause.marca = marca;

		let orderClause = [];
		if (ordenarPor === 'menorMayor') orderClause = [['precio', 'ASC']];
		else if (ordenarPor === 'mayorMenor') orderClause = [['precio', 'DESC']];
		else if (ordenarPor === 'novedades') orderClause = [['createdAt', 'DESC']];
...
// Iniciar el servidor y precargar datos
app.listen(4001, async () => {
	await preloadUser(); 
	await preloadZapatillas(); 
